...
const APP_LABEL = "com.proxy-max.autostart";
 ...
 *   2. `process.argv[1]` if it's our cli.js — true when proxy-max is currently
...
function isAutoStartEnabled() {
  ...
  try {
    if (platform === "darwin") {
      ...
      try {
        execFileSync("/bin/launchctl", ["list", APP_LABEL], {
 ...
 * SIGTERM to the running process. When the running proxy-max cli.js was itself
...
function isAgentSelfMacOS() {
  try {
    const output = execFileSync("/bin/launchctl", ["list", APP_LABEL], {
      encoding: "utf8",
...
function enableMacOS(cliPath) {
  ...
  const logDir = path.join(os.homedir(), "Library", "Logs", APP_NAME);
...
<dict>
    ...
    <array>
        ...
        <string>${escapeXml(routerScript)}</string>
        <string>--tray</string>
  ...
  fs.writeFileSync(plistPath, plistContent, { mode: 0o600 });
  ...
  try {
    execFileSync("/bin/launchctl", ["unload", plistPath], { stdio: "ignore", timeout: 5000 });
  } catch (e) {}
  try {
    execFileSync("/bin/launchctl", ["load", "-w", plistPath], { stdio: "ignore", timeout: 5000 });
  } catch (e) {
...
function enableWindows(cliPath) {
  ...
  // `proxy-max.cmd` lookup that depended on the npm prefix path.
...
Type=Application
Name=Proxy Max
Comment=Proxy Max API Proxy
Exec=${quoteDesktopArg(nodePath)} ${quoteDesktopArg(routerScript)} --tray --skip-update
Hidden=false
NoDisplay=false
X-GNOME-Autostart-enabled=true
`;
  fs.writeFileSync(desktopPath, desktopContent, { mode: 0o600 });
  return true;
}
