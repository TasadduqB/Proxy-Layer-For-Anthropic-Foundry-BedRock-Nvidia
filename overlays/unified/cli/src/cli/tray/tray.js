...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `Proxy Max (Port ${port})`, tooltip: "Server is running", enabled: false },
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[proxy-max] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[proxy-max] tray init error: ${err.message}\n`);
    return null;
  }
}
...
function openBrowser(url) {
  ...
  if (file) execFile(file, args, { windowsHide: true, timeout: 10_000, maxBuffer: 64 * 1024 }, () => {});
}
