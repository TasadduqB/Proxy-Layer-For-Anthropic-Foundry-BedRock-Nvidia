...
async function buildCodexHeader() {
  ...
  const { installed, hasProxyMax, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasProxyMax, settings } = result.data;
  ...
  // Extract Proxy Max custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:Proxy Max-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Proxy Max provider config
  const provider = settings?.models?.providers?.["proxy-max"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("proxy-max/") ? primary.replace("proxy-max/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasProxyMax, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Proxy Max\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
