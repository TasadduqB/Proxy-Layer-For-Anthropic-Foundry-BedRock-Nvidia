/**
 * `proxy-max xai video` — generate a Grok Imagine video through the local
 * proxy-max gateway and save the result as an MP4 file.
 *
...
const HELP = `
Usage: proxy-max xai video --prompt "..." [options]

Generate a Grok Imagine video via your local proxy-max gateway
...
Options:
  ...
  --api-key <key>         proxy-max API key (or env NINE_ROUTER_API_KEY)
...
async function run(argv) {
  ...
  try {
    ...
    const connectionId = create.headers["x-proxy-max-connection-id"] || null;
