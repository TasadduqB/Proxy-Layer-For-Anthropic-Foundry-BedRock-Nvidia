...
function normalizeMappedTokens(providerName, mapped) {
  if (!mapped || typeof mapped !== "object") {
    throw new Error(`${providerName} token exchange returned an invalid payload`);
  }
  if (typeof mapped.accessToken !== "string" || !mapped.accessToken.trim()) {
    throw new Error(`${providerName} token exchange returned no access token`);
  }
  if (mapped.expiresIn !== undefined && mapped.expiresIn !== null) {
    const expiresIn = Number(mapped.expiresIn);
    if (!Number.isFinite(expiresIn) || expiresIn <= 0) {
      delete mapped.expiresIn;
    } else {
      mapped.expiresIn = Math.max(1, Math.floor(expiresIn));
    }
  }
  return mapped;
}

export function normalizeGitLabBaseUrl(rawBaseUrl) {
  const value = String(rawBaseUrl || GITLAB_CONFIG.defaultBaseUrl).trim();
...
async function fetchGitLab(url, init) {
  return fetchUrlWithPolicy(url, init, {
    maxRedirects: 0,
...
const PROVIDERS = {
  ...
  gitlab: {
    ...
    buildAuthUrl: (config, redirectUri, state, codeChallenge, meta = {}) => {
      ...
      if (!clientId) throw new Error("GitLab OAuth client ID is required");
      const params = new URLSearchParams({
        client_id: clientId,
        redirect_uri: redirectUri,
        response_type: "code",
    ...
    exchangeToken: async (config, code, redirectUri, codeVerifier, state, meta = {}) => {
      ...
      const response = await fetchGitLab(`${baseUrl}${config.tokenUrlPath}`, {
        method: "POST",
      ...
      // Fetch user info
      const userRes = await fetchGitLab(`${baseUrl}${config.userInfoUrlPath}`, {
        headers: { Authorization: `Bearer ${tokens.access_token}` },
      });
    ...
    mapTokens: (tokens) => {
      const email = tokens._user?.email || tokens._user?.public_email || null;
      const username = tokens._user?.username || "";
      const name = tokens._user?.name || "";
      return {
        accessToken: tokens.access_token,
        refreshToken: tokens.refresh_token,
        expiresIn: tokens.expires_in,
        ...
        displayName: name || username || email || undefined,
        providerSpecificData: {
          username,
          email: email || "",
          name,
...
export async function generateAuthData(providerName, redirectUri, meta) {
  ...
  if (provider.flowType === "device_code" || provider.flowType === "import_token") {
    // Device/import flows don't have an authorization URL upfront.
    authUrl = null;
  ...
  if (provider.flowType !== "device_code" && provider.flowType !== "import_token") {
    registerAuthorizationSession(providerName, authData, meta);
  }
  return authData;
}
...
export async function exchangeTokens(providerName, code, redirectUri, codeVerifier, state, meta) {
  ...
  return normalizeMappedTokens(providerName, provider.mapTokens(tokens, extra));
}
...
export async function pollForToken(providerName, deviceCode, codeVerifier, extraData) {
  ...
  if (result.ok) {
    ...
    if (result.data.access_token) {
      ...
      const tokens = normalizeMappedTokens(providerName, provider.mapTokens(result.data, extra));
  ...
  const error = result?.data?.error || "device_flow_failed";
  return {
    success: false,
    error,
    errorDescription: result?.data?.error_description || result?.data?.message,
