...
const previousDefaultRegExp = /^# proxy-max-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# proxy-max-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via Proxy Max gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# proxy-max-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# proxy-max-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
