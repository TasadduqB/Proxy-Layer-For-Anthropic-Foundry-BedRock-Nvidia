...
export async function saveRequestUsage(entry) {
  try {
    ...
    db.transaction(() => {
      // A timestamp + model + token tuple is not an identity: parallel requests
      // routinely complete in the same millisecond with identical usage. The
      // upstream heuristic collapsed those legitimate events (often 99/100).
      // Only an explicit request/event id is idempotent; NULL requestId values
      // are intentionally all accepted by SQLite's partial unique index.
      const insertResult = db.run(
        `INSERT OR IGNORE INTO usageHistory(timestamp, provider, model, connectionId, apiKey, endpoint, requestId, promptTokens, completionTokens, cost, status, tokens, meta) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          entry.timestamp, entry.provider || null, entry.model || null,
          entry.connectionId || null, entry.apiKey || null, entry.endpoint || null, requestId,
          promptTokens, completionTokens, entry.cost || 0, entry.status || "ok",
          stringifyJson(tokens), stringifyJson({}),
        ]
      );
      if (!insertResult?.changes) return;
