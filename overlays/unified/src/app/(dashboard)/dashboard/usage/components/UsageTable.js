...
/**
 * Render token details or the server-calculated total cost.
 *
 * Cached read/write rates vary by provider and model. Avoid deriving a
 * component cost by spreading the total evenly across tokens because that
 * would make discounted cache reads look as expensive as fresh input.
 */
function ValueCells({ item, viewMode, isSummary = false }) {
  if (viewMode === "tokens") {
    const cachedRead = item.cachedTokens || 0;
    const cacheWrite = item.cacheCreationTokens || 0;
    const reportedInput = item.promptTokens || 0;
    ...
    const freshInput = Math.max(0, totalInput - cachedPortion);
    const totalProcessed = totalInput + (item.completionTokens || 0);
    return (
      <>
        <td className="px-6 py-3 text-right text-text-muted">
          {isSummary && item.promptTokens === undefined ? "—" : fmt(freshInput)}
        </td>
        <td className="px-6 py-3 text-right text-text-muted">
          {cachedRead ? fmt(cachedRead) : "—"}
        </td>
        <td className="px-6 py-3 text-right text-text-muted">
...
export default function UsageTable({
  ...
  const valueColumns = useMemo(() => {
    if (viewMode === "tokens") {
      return [
        { field: "freshInputTokens", label: "Fresh Input" },
        { field: "cachedTokens", label: "Cache Read" },
        { field: "cacheCreationTokens", label: "Cache Write" },
        { field: "promptTokens", label: "Total Input" },
        { field: "completionTokens", label: "Output Tokens" },
        { field: "totalTokens", label: "Total Processed" },
      ];
    }
    return [{ field: "cost", label: "Est. Total Cost" }];
  }, [viewMode]);
