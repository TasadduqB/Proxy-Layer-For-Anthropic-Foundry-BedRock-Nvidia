...
import ProviderLaunchCommands from "./ProviderLaunchCommands";
...
// These providers expose an account-backed catalog that changes independently
// of the bundled fallback registry.
const LIVE_CATALOG_PROVIDERS = new Set(["azure", "bedrock", "cursor", "nvidia"]);
...
export default function ProviderDetailPage() {
  ...
  const launchModels = (() => {
    ...
    const add = (modelId) => {
      if (!modelId) return;
      const suffix = resolveThinkingSuffix(modelId);
      const fullModel = `${providerDisplayAlias}/${modelId}${suffix ? `(${suffix})` : ""}`;
      if (seen.has(fullModel)) return;
      seen.add(fullModel);
      ids.push(fullModel);
    };

    if (!isCompatible) {
      for (const model of [...models, ...kiloFreeModels]) {
        const kind = getModelKind(model);
        if ((!kind || kind === "llm") && !disabledModelIds.includes(model.id)) add(model.id);
      }
    }
  ...
  // These account-backed catalogs change by region, deployment, or account.
  // The static registry remains the fallback while discovery is unavailable.
  useEffect(() => {
    if (!LIVE_CATALOG_PROVIDERS.has(providerId)) {
      setLiveModels([]);
      return;
    }
    ...
    let cancelled = false;
    const catalogUrl = providerId === "nvidia"
      ? "/api/providers/nvidia-catalog"
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:gap-8 sm:px-0">
      ...
      <ProviderLaunchCommands providerName={providerInfo.name} models={launchModels} />
