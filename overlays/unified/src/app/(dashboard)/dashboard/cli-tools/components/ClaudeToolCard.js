...
const serverAliasesFor = (model) => [...new Set(model.serverAliases || [model.alias])];
...
export default function ClaudeToolCard({
  ...
  const [modelAliasesLoaded, setModelAliasesLoaded] = useState(false);
  ...
  const fetchModelAliases = async () => {
    try {
      ...
      setModelAliasesLoaded(true);
    }
  };

  useEffect(() => {
    if (claudeStatus?.installed && modelAliasesLoaded && !hasInitializedModels.current) {
      hasInitializedModels.current = true;
      const env = claudeStatus.settings?.env || {};

      tool.defaultModels.forEach((model) => {
        const serverTarget = serverAliasesFor(model).map((alias) => modelAliases[alias]).find(Boolean);
        const value = serverTarget || (model.envKey ? env[model.envKey] : "") || model.defaultValue || "";
        if (value) {
  ...
  }, [claudeStatus, apiKeys, modelAliases, modelAliasesLoaded, tool.defaultModels, onModelMappingChange]);
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      // Get key from dropdown, fallback to first key or sk_proxy-max for localhost
      const keyToUse = selectedApiKey?.trim()
        || (apiKeys?.length > 0 ? apiKeys[0].key : null)
        || (!cloudEnabled ? "sk_proxy-max" : null);
      ...
      const res = await fetch("/api/cli-tools/claude-settings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ env, exaMcpEnabled, clearModelEnv: true }),
      });
      const data = await res.json();
      if (res.ok) {
        const aliasUpdates = [];
        for (const model of tool.defaultModels) {
          const target = (modelMappings[model.alias] || "").trim();
          for (const alias of serverAliasesFor(model)) {
            aliasUpdates.push(fetch(target ? "/api/models/alias" : `/api/models/alias?alias=${encodeURIComponent(alias)}`, {
              method: target ? "PUT" : "DELETE",
              headers: target ? { "Content-Type": "application/json" } : undefined,
              body: target ? JSON.stringify({ alias, model: target }) : undefined,
            }));
          }
        }
        const aliasResponses = await Promise.all(aliasUpdates);
        if (aliasResponses.some((response) => !response.ok)) {
          throw new Error("Claude endpoint saved, but one or more server model routes failed");
        }
  ...
  const handleResetSettings = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        const aliases = tool.defaultModels.flatMap(serverAliasesFor);
        await Promise.all(aliases.map((alias) => fetch(`/api/models/alias?alias=${encodeURIComponent(alias)}`, { method: "DELETE" })));
  ...
  const getManualConfigs = () => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim())
      ? selectedApiKey
      : (!cloudEnabled ? "sk_proxy-max" : "<API_KEY_FROM_DASHBOARD>");
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if proxy-max is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingClaude && claudeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Model Mappings */}
                <div className="rounded-lg border border-primary/15 bg-primary/[0.04] px-3 py-2 text-xs leading-relaxed text-text-muted">
                  These are Proxy Max server routes, not Claude environment overrides. Targets may be provider/model IDs or combo names; fallback and round-robin run on the server.
                </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!claudeStatus?.hasProxyMax} loading={restoring}>
