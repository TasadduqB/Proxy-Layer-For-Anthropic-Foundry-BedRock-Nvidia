...
const hasProxyMaxConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["proxy-max"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["proxy-max"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasProxyMax: hasProxyMaxConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("proxy-max/") ? config.model.replace(/^proxy-max\//, "") : null,
...
// POST - Apply Proxy Max as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_proxy-max";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing proxy-max provider entry and its models
    const existingProvider = config.provider["proxy-max"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["proxy-max"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `proxy-max/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `proxy-max/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("proxy-max/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Proxy Max provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["proxy-max"]?.models) {
      delete config.provider["proxy-max"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["proxy-max"].models).length === 0) {
        delete config.provider["proxy-max"];
        if (config.model?.startsWith("proxy-max/")) delete config.model;
      } else if (config.model === `proxy-max/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["proxy-max"].models);
        config.model = `proxy-max/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire proxy-max provider
      if (config.provider) delete config.provider["proxy-max"];
      if (config.model?.startsWith("proxy-max/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("proxy-max/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Proxy Max settings removed from OpenCode",
    });
  } catch (error) {
