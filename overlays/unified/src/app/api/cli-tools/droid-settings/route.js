...
// Check if settings has Proxy Max customModels
const hasProxyMaxConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Proxy Max"));
};
...
// POST - Update Proxy Max customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Proxy Max configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Proxy Max"));
...
// DELETE - Remove Proxy Max customModels only (keep other settings)
