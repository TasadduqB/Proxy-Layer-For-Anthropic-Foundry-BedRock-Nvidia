...
export async function GET() {
  try {
    ...
    const hasProxyMax = !!(settings?.env?.ANTHROPIC_BASE_URL);
    const claudeJson = await readClaudeJson();

    return NextResponse.json({
      installed: true,
      settings: settings,
      hasProxyMax: hasProxyMax,
...
export async function POST(request) {
  try {
    const { env, exaMcpEnabled, clearModelEnv = false } = await request.json();
    ...
    // Claude Code appends /v1/messages itself. Preserve any intentional
    // gateway path, but do not add an OpenAI-style /v1 suffix here.
    ...
    if (clearModelEnv) {
      for (const key of CLAUDE_MODEL_ENV_KEYS) delete existingEnv[key];
    }

    // Model routing belongs to Proxy Max; Claude receives only endpoint/auth.
    const newSettings = {
      ...currentSettings,
