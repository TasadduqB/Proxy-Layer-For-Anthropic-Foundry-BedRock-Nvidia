...
const hasProxyMaxConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Proxy Max");
};

const getProxyMaxEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Proxy Max") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getProxyMaxEntry(config);
...
// POST - Apply Proxy Max config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Proxy Max entry or append
    const idx = config.findIndex((e) => e.name === "Proxy Max");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Proxy Max entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Proxy Max");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Proxy Max removed from Copilot config",
    });
  } catch (error) {
