...
const hasProxyMaxConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["proxy-max"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("proxy-max");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasProxyMax: hasProxyMaxConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "Proxy Max", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Proxy Max settings removed from Kilo Code" });
  } catch (error) {
