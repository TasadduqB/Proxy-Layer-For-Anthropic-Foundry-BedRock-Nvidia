...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-proxy-max.env");
};
...
const hasProxyMaxConfig = (config) => {
  ...
  if (providers["proxy-max"]) return true;
...
export async function GET() {
  ...
  const hasProxyMax = hasProxyMaxConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_PROXY_MAX_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile proxy-max",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["proxy-max"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_PROXY_MAX_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "proxy-max configuration removed from jcode",
    });
  } catch (error) {
