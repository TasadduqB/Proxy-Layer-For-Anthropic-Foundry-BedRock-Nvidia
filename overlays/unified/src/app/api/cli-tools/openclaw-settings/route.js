...
// Check if settings has Proxy Max config
const hasProxyMaxConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["proxy-max"];
};

// Read per-agent models.json and return current model id (without "proxy-max/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["proxy-max"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasProxyMax: hasProxyMaxConfig(settings),
...
// POST - Update Proxy Max settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `proxy-max/${model}`;

    // Remove all old proxy-max/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("proxy-max/"))
    ...
    // Add fresh proxy-max models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`proxy-max/${m}`] = {};
    });

    // Remove old proxy-max model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("proxy-max/")) {
    ...
    // Update models.providers.proxy-max with all models
    settings.models.providers["proxy-max"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `proxy-max/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Proxy Max settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Proxy Max from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["proxy-max"];
    ...
    // Remove proxy-max models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("proxy-max/"));
    ...
    // Reset agents.defaults.model.primary if it uses proxy-max
    if (settings.agents?.defaults?.model?.primary?.startsWith("proxy-max/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Proxy Max settings removed successfully",
    });
  } catch (error) {
