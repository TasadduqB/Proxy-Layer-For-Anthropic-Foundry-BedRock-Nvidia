...
import { isAdmin as isWindowsAdmin } from "@/mitm/winElevated.js";
...
function normalizeMitmRouterBaseUrlInput(input) {
  ...
  if (u.username || u.password || u.hash) throw new Error("MITM router URL must not contain credentials or a fragment");
  return t;
}
...
export async function POST(request) {
  try {
    ...
    if (typeof apiKey !== "string" || !apiKey.trim() || /[\0\r\n]/.test(apiKey) || Buffer.byteLength(apiKey) > 16_384) {
      return NextResponse.json({ error: "Invalid apiKey" }, { status: 400 });
    }
    if (sudoPassword != null && (typeof sudoPassword !== "string" || /[\0\r\n]/.test(sudoPassword) || Buffer.byteLength(sudoPassword) > 4096)) {
      return NextResponse.json({ error: "Invalid sudoPassword" }, { status: 400 });
    }
    ...
    if (!checkPrivilege(pwd)) {
      return NextResponse.json(
        { error: isWin ? "Administrator required — restart Proxy Max as Administrator" : "Root or sudo password required to start MITM" },
        { status: 403 }
      );
    }
    ...
    const result = await startServer(apiKey, pwd, forceKillPort443 === true);
    if (!isWin) setCachedPassword(pwd);

    return NextResponse.json({ success: true, running: result.running, pid: result.pid });
  } catch (error) {
    const safeMessage = String(error?.message || "Failed to start MITM server").replace(/[\r\n\t]+/g, " ").slice(0, 500);
    console.log("Error starting MITM server:", safeMessage);
    if (error.code === "PORT_443_BUSY") {
      return NextResponse.json(
        { error: safeMessage, code: "PORT_443_BUSY", portOwner: error.portOwner },
        { status: 409 }
      );
    }
...
export async function PATCH(request) {
  try {
    ...
    if (!checkPrivilege(pwd)) {
      return NextResponse.json(
        { error: isWin ? "Administrator required — restart Proxy Max as Administrator" : "Root or sudo password required to modify DNS" },
        { status: 403 }
      );
    }
