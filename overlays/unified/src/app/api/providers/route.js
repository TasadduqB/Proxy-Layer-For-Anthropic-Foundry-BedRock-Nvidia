...
import { redactCredentialObject } from "@/lib/security/redact";
...
export async function POST(request) {
  try {
    ...
    if (isOpenAICompatibleProvider(provider)) {
      ...
      try {
        const azureData = normalizeAzureProviderSpecificData(providerSpecificData || {}, {
          credentials: { apiKey, providerSpecificData: providerSpecificData || {} },
          model: defaultModel || providerSpecificData?.deployment,
        });
        providerSpecificData = { ...(providerSpecificData || {}), ...azureData };
      } catch {
      ...
      try {
        providerSpecificData = {
          ...(providerSpecificData || {}),
          ...normalizeBedrockProviderSpecificData(providerSpecificData || {}, { requireSecret: true }),
        };
      } catch {
    ...
    const result = redactCredentialObject(newConnection);
