...
import { resolveAzureModels } from "@/lib/providers/azureModels";
import { resolveBedrockModels } from "@/lib/providers/bedrockModels";
...
// NVIDIA can retain retired entries in /v1/models after their inference
// deployment has been removed. Those entries consistently return an upstream
// 404 from /v1/chat/completions and must not be offered by the dashboard.
const UNAVAILABLE_PROVIDER_MODELS = {
  nvidia: new Set(["nvidia/nemotron-mini-4b-instruct"]),
};

const filterUnavailableModels = (provider, models) => {
  const unavailable = UNAVAILABLE_PROVIDER_MODELS[provider];
  if (!unavailable || !Array.isArray(models)) return models;
...
const PROVIDER_MODELS_CONFIG = {
  ...
  azure: {
    customResolver: async (connection) => {
      ...
      try {
        const result = await resolveAzureModels(connection, {
          forceRefresh: true,
          proxyOptions: proxy,
        });
        return result.models.length ? result : {
          models: getStaticProviderModels("azure"),
          warning: result.warning || "Azure AI Foundry returned no models; using configured models.",
        };
      } catch (error) {
        return {
          models: getStaticProviderModels("azure"),
          warning: `Azure AI Foundry catalog unavailable: ${error.message}`,
        };
      }
    },
  },
  bedrock: {
    customResolver: async (connection) => {
      const proxy = await resolveConnectionProxyConfig(connection.providerSpecificData || {});
      try {
        const result = await resolveBedrockModels(connection, {
          forceRefresh: true,
          proxyOptions: proxy,
        });
        return result.models.length ? result : {
          models: getStaticProviderModels("bedrock"),
          warning: result.warning || "AWS Bedrock returned no models; using the bundled fallback catalog.",
        };
      } catch (error) {
        return {
          models: getStaticProviderModels("bedrock"),
          warning: `AWS Bedrock catalog unavailable: ${error.message}`,
        };
      }
    },
  },
...
export async function GET(request, { params }) {
  try {
    ...
    const data = await response.json();
    const models = filterUnavailableModels(connection.provider, config.parseResponse(data));
