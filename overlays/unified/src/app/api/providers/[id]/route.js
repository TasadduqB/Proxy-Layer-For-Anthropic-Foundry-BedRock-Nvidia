...
import { MASKED_VALUE, containsMaskedValue, mergeMaskedSecrets, redactCredentialObject } from "@/lib/security/redact";
import { normalizeAzureProviderSpecificData } from "open-sse/executors/azure.js";
import { normalizeBedrockProviderSpecificData } from "open-sse/executors/bedrock.js";
...
export async function GET(request, { params }) {
  try {
    ...
    const result = redactCredentialObject(connection);
...
export async function PUT(request, { params }) {
  try {
    ...
    if (apiKey && apiKey !== MASKED_VALUE && existing.authType === "apikey") updateData.apiKey = apiKey;
    ...
    if (
      ...
      updateData.providerSpecificData = mergeMaskedSecrets(providerSpecificData || {}, existing.providerSpecificData || {});

      if (existing.provider === "azure") {
        try {
          const azureData = normalizeAzureProviderSpecificData(updateData.providerSpecificData, {
            credentials: {
              apiKey: apiKey || existing.apiKey,
              accessToken: existing.accessToken,
              providerSpecificData: updateData.providerSpecificData,
            },
            model: defaultModel || existing.defaultModel || updateData.providerSpecificData.deployment,
          });
          updateData.providerSpecificData = { ...updateData.providerSpecificData, ...azureData };
        } catch {
      ...
      if (existing.provider === "bedrock") {
        try {
          updateData.providerSpecificData = {
            ...updateData.providerSpecificData,
            ...normalizeBedrockProviderSpecificData(updateData.providerSpecificData, { requireSecret: true }),
          };
        } catch {
          return NextResponse.json({ error: "Invalid AWS Bedrock configuration" }, { status: 400 });
        }
      }

      if (proxyConfig.hasAnyProxyField) {
        updateData.providerSpecificData.connectionProxyEnabled = proxyConfig.connectionProxyEnabled;
        if (!containsMaskedValue(proxyConfig.connectionProxyUrl)) {
    ...
    // Hide sensitive fields
    const result = redactCredentialObject(updated);
    delete result.apiKey;
    delete result.accessToken;
    delete result.refreshToken;
    delete result.idToken;
