...
const OAUTH_TEST_CONFIG = {
  ...
  github: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    extraHeaders: { "User-Agent": "Proxy Max", "Accept": "application/vnd.github+json" },
  },
  iflow: {
  ...
  try {
    switch (connection.provider) {
      ...
      case "azure": {
        try {
          const psd = connection.providerSpecificData || {};
          const { response: res } = await executeAzureProbe({
            apiKey: connection.apiKey,
            accessToken: connection.accessToken,
            providerSpecificData: psd,
            model: connection.defaultModel || psd.deployment || "gpt-4",
          }, {
            fetchFn: (url, options) => fetchWithConnectionProxy(url, options, effectiveProxy),
            signal: AbortSignal.timeout(10000),
          });
          const valid = ![401, 403, 404].includes(res.status);
          return { valid, error: valid ? null : "Invalid Azure credential or configuration" };
        } catch {
          return { valid: false, error: "Invalid Azure credential or configuration" };
        }
      }
      case "openai": {
