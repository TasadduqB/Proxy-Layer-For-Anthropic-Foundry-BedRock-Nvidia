...
import { executeAzureProbe } from "@/lib/providers/azureProbe";
import { probeBedrockConnection } from "@/lib/providers/bedrockProbe";
...
export async function POST(request) {
  try {
    ...
    const { apiKey, accessToken, providerSpecificData } = body;

    const isNoAuth = AI_PROVIDERS[provider]?.noAuth === true;
    const hasCredential = Boolean(apiKey || (provider === "azure" && accessToken));
    if (!provider || (!hasCredential && provider !== "ollama-local" && !isNoAuth)) {
    ...
    try {
      ...
      if (provider === "bedrock") {
        if (!providerSpecificData?.secretAccessKey) {
          return NextResponse.json({ valid: false, error: "AWS secret access key is required" });
        }
        ...
        const result = await probeBedrockConnection({
          ...
          fetchProxyOptions: effectiveProxy,
      ...
      if (provider === "azure") {
        try {
          const effectiveProxy = await resolveConnectionProxyConfig(providerSpecificData || {});
          const { response: azureRes } = await executeAzureProbe({
            apiKey,
            accessToken,
            providerSpecificData: providerSpecificData || {},
            model: body.model || providerSpecificData?.deployment || "gpt-4",
          }, {
            fetchFn: proxyAwareFetch,
            proxyOptions: effectiveProxy,
            signal: AbortSignal.timeout(10000),
          });
          isValid = ![401, 403, 404].includes(azureRes.status);
          return NextResponse.json({
            valid: isValid,
            error: isValid ? null : "Invalid Azure credential or configuration",
          });
        } catch {
          return NextResponse.json({ valid: false, error: "Invalid Azure credential or configuration" });
        }
      }
