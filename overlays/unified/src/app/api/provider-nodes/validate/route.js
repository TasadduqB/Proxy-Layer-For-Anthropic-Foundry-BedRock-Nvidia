...
import { fetchUrlWithPolicy } from "@/shared/utils/ssrfGuard.js";
...
export async function POST(request) {
  try {
    ...
    // Local authenticated users may validate self-hosted nodes; tunneled/remote
    // callers get DNS-aware checks on the initial URL and every redirect hop.
    const allowPrivate = isLocalRequest(request);
    const fetchWithTimeout = (url, options, timeout = 10000) =>
      fetchUrlWithPolicy(url, options, { allowPrivate, timeoutMs: timeout });
