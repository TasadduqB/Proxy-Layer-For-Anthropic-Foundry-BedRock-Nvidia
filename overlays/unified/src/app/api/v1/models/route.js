...
const LIVE_MODEL_RESOLVERS = {
  azure: async (conn) => {
    ...
    return resolveAzureModels(conn, { proxyOptions });
  },
  bedrock: async (conn) => {
    const proxyOptions = await resolveConnectionProxyConfig(conn.providerSpecificData || {});
    return resolveBedrockModels(conn, { proxyOptions });
  },
  nvidia: async (conn) => {
    const ids = await getNvidiaCatalogModels(conn);
    return ids.length ? {
      models: ids.map((id) => ({
        // The provider id and NVIDIA's organization namespace are both
        // `nvidia`. Prefix nested ids once so the generic normalization below
        // preserves the upstream id as nvidia/nvidia/<model>.
        id: id.startsWith("nvidia/") ? `nvidia/${id}` : id,
      })),
    } : null;
  },
  kiro: async (conn) => {
...
// and break recursive loops between proxy-max instances connected to each other.
...
function inferKindFromUnknownModelId(modelId) {
  ...
  if (/(?:^|[-_/])(?:asr|stt)(?:$|[-_/])|transcri/.test(lower)) return "stt";
...
export async function buildModelsList(kindFilter, options = {}) {
  ...
  // proxy-max instance's fetchCompatibleModelIds — skip dynamic fetch to break
...
export async function GET(request) {
  try {
    // Detect cross-instance recursive /models fetch (another proxy-max fetching our /models)
