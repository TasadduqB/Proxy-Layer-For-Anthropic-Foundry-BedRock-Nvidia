...
export async function GET(request, { params }) {
  try {
    ...
    return NextResponse.json({ proxyPool: redactCredentialObject(proxyPool) });
  } catch (error) {
...
export async function PUT(request, { params }) {
  try {
    ...
    if (containsMaskedValue(normalized.updates.proxyUrl)) delete normalized.updates.proxyUrl;

    const updated = await updateProxyPool(id, normalized.updates);
    return NextResponse.json({ proxyPool: redactCredentialObject(updated) });
  } catch (error) {
