...
export async function GET(request) {
  try {
    ...
    const proxyPools = (await getProxyPools(filter)).map((pool) => redactCredentialObject(pool));
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({ proxyPool: redactCredentialObject(proxyPool) }, { status: 201 });
  } catch (error) {
