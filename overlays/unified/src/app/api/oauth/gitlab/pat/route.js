...
import { normalizeGitLabBaseUrl } from "@/lib/oauth/providers";
...
export async function POST(request) {
  try {
    ...
    let base;
    try {
      base = normalizeGitLabBaseUrl(baseUrl?.trim() || GITLAB_DEFAULT_BASE);
    } catch (error) {
      return NextResponse.json({ error: error.message }, { status: 400 });
    }

    // Verify token by fetching current user
    const userRes = await fetchUrlWithPolicy(`${base}/api/v4/user`, {
