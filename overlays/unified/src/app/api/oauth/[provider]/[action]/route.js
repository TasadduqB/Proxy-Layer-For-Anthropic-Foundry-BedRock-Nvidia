...
export async function GET(request, { params }) {
  try {
    ...
    if (action === "start-proxy") {
      ...
      const parsedAppPort = Number(appPort);
      if (!Number.isInteger(parsedAppPort) || parsedAppPort < 1 || parsedAppPort > 65535) {
      ...
      const result = provider === "xai"
        ? await startXaiProxy(parsedAppPort)
        : await startCodexProxy(parsedAppPort);
...
export async function POST(request, { params }) {
  try {
    ...
    if (action === "exchange") {
      ...
      // Raw JWT import is a Codex-only convenience. Treating an arbitrary JWT
      // as credentials for any provider bypassed that provider's OAuth flow.
      const looksLikeJwt = typeof code === "string" && /^eyJ[^.]*\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]*$/.test(code);
      if (looksLikeJwt && provider !== "codex") {
        return NextResponse.json({ error: "Raw JWT import is only supported for codex" }, { status: 400 });
      }
      if (looksLikeJwt && provider === "codex") {
      ...
      const authorizationSession = consumeAuthorizationSession(provider, {
        state,
        redirectUri,
        codeVerifier,
        meta,
