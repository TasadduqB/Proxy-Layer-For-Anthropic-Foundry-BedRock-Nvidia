...
function withConnectionHeader(response, connectionId) {
  ...
  headers.set("x-proxy-max-connection-id", String(connectionId));
...
export async function handleVideoGet(request, requestId) {
  ...
  const credentials = await getProviderCredentials(provider, null, null, {
    ...
    // Polls are bound to the account that created the upstream job. Falling
    // back to a different account can never retrieve that job and may expose a
    // same-id job from another account, so an explicit pin must be strict.
