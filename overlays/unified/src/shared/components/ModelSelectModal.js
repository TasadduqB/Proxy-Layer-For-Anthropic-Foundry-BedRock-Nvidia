...
export default function ModelSelectModal({
  ...
  // NVIDIA NIM's catalog is large and account-specific (100+ chat models beyond
  // the curated static list). Fetch it live so the full catalog is selectable,
  // same rationale as Cursor above.
  const hasNvidiaConnection = useMemo(
    () => activeProviders.some((provider) => provider.provider === "nvidia"),
    [activeProviders],
  );

  useEffect(() => {
    if (!isOpen || !hasNvidiaConnection) {
      setNvidiaModels([]);
      return undefined;
    }

    let cancelled = false;
    fetch("/api/providers/nvidia-catalog", { cache: "no-store" })
      .then((response) => (response.ok ? response.json() : null))
      .then((data) => {
        if (cancelled || !data) return;
        const models = Array.isArray(data.models) ? data.models : [];
        setNvidiaModels(models.filter((model) => model?.id));
      })
      .catch((error) => {
        // Do not hide the static curated list when the live catalog is unavailable.
        console.warn("Unable to load NVIDIA NIM catalog for selector:", error);
        if (!cancelled) setNvidiaModels([]);
      });

    return () => { cancelled = true; };
  }, [isOpen, hasNvidiaConnection]);
  ...
  const groupedModels = useMemo(() => {
    ...
    sortedProviderIds.forEach((providerId) => {
      ...
      if (providerInfo.passthroughModels) {
        ...
        const curatedModels = getModelsByProviderId(providerId);
        const hardcodedModels = providerId === "cursor" && cursorModels.length > 0
          ? cursorModels
          : providerId === "nvidia" && nvidiaModels.length > 0
            // Merge: keep curated entries (they carry kind tags for tts/stt/embedding),
            // add the rest of the live catalog as plain chat models.
            ? [...curatedModels, ...nvidiaModels.filter((m) => !curatedModels.some((c) => c.id === m.id))]
            : curatedModels;
    ...
    return groups;
  }, [filteredActiveProviders, modelAliases, allProviders, providerNodes, customModels, disabledModels, kindFilter, activeProviders, cursorModels, nvidiaModels]);
