...
const getPageInfo = (pathname) => {
  ...
  if (pathname.includes("/basic-chat"))
    return {
      title: "Playground",
      description: "Test routed models and inspect streaming responses",
      icon: "chat",
      breadcrumbs: [],
    };
  if (pathname.includes("/pxpipe"))
    return {
      title: "PXPIPE",
      description: "Manage the local image generation pipeline",
      icon: "image",
      breadcrumbs: [],
    };
  ...
  if (pathname.includes("/mitm"))
    return {
      title: "MITM Proxy",
      description: "Intercept CLI tool traffic and route through Proxy Max",
      icon: "security",
      breadcrumbs: [],
    };
  ...
  if (pathname.includes("/skills"))
    return {
      title: "Agent Skills",
      description: "Copy a link and paste to your AI to use Proxy Max — no install needed",
      icon: "extension",
      breadcrumbs: [],
    };
  ...
  if (pathname === "/dashboard")
    return {
      title: "Overview",
      description: "Gateway health, routing coverage, and recent activity",
...
export default function Header({ onMenuClick, showMenuButton = true }) {
  ...
  return (
    <header className="app-header z-20 flex shrink-0 items-center justify-between gap-3 border-b px-4 py-3 lg:px-8">
      {/* Mobile menu button */}
      <div className="flex items-center gap-3 lg:hidden shrink-0">
        {showMenuButton && (
          <button
            type="button"
            onClick={onMenuClick}
            className="icon-tile flex size-9 items-center justify-center rounded-[11px] text-text-main transition-all hover:scale-105 hover:text-primary"
        ...
        {breadcrumbs.length > 0 ? (
          <div className="flex items-center gap-2">
            {breadcrumbs.map((crumb, index) => (
              ...
              >
                {index > 0 && (
                  <span aria-hidden="true" className="material-symbols-outlined text-text-muted text-base">
                    chevron_right
                  </span>
                )}
                {crumb.href ? (
          ...
          <div>
            <div className="flex items-center gap-2">
              {icon && (
                <span className="icon-tile size-9 rounded-[11px]" aria-hidden="true">
                  <span className="material-symbols-outlined text-[19px]">{icon}</span>
                </span>
              )}
              <h1 className="truncate text-base font-semibold tracking-[-0.02em] lg:text-2xl">
                {translate(title)}
              </h1>
            </div>
            {description && (
