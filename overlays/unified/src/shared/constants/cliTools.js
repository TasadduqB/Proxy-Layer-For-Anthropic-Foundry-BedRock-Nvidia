...
export const CLI_TOOLS = {
  claude: {
    ...
    defaultModels: [
      { id: "fable", name: "Claude Fable", alias: "fable", envKey: "ANTHROPIC_DEFAULT_FABLE_MODEL", defaultValue: "claude-auto", serverAliases: ["fable", "claude-fable-5"] },
      { id: "opus", name: "Claude Opus", alias: "opus", envKey: "ANTHROPIC_DEFAULT_OPUS_MODEL", defaultValue: "claude-auto", serverAliases: ["opus", "opusplan", "claude-opus-4-8", "claude-opus-4-6", "claude-opus-4-6-thinking"] },
      { id: "sonnet", name: "Claude Sonnet", alias: "sonnet", envKey: "ANTHROPIC_DEFAULT_SONNET_MODEL", defaultValue: "claude-auto", serverAliases: ["default", "sonnet", "claude-sonnet-5", "claude-sonnet-4-6", "claude-sonnet-4-5"] },
      { id: "haiku", name: "Claude Haiku", alias: "haiku", envKey: "ANTHROPIC_DEFAULT_HAIKU_MODEL", defaultValue: "claude-auto", serverAliases: ["haiku", "claude-haiku-4-5-20251001", "claude-haiku-4-5"] },
    ],
  },
  openclaw: {
    id: "openclaw",
  ...
  amp: {
    ...
    notes: [
      { type: "info", text: "Use Proxy Max model aliases to keep Amp shorthand mappings stable across provider updates." },
    ...
    guideSteps: [
      ...
      { step: 5, title: "Add Shorthands", desc: "Map Amp shorthand names such as g25p or cs45 to Proxy Max aliases in your local config." },
    ],
    codeBlock: {
      language: "bash",
...
# cs45 -> cc/claude-sonnet-4-5-20250929`,
  ...
  qwen: {
    ...
    color: "#10B981",
    description: "Alibaba Qwen Code CLI — supports OpenAI, Anthropic & Gemini providers via Proxy Max",
    docsUrl: "https://qwenlm.github.io/qwen-code-docs/en/users/configuration/model-providers/",
    configType: "guide",
    defaultCommand: "qwen",
    notes: [
      { type: "info", text: "Qwen Code supports multiple provider types (openai, anthropic, gemini) via modelProviders in settings.json. Proxy Max works as an OpenAI-compatible endpoint." },
      { type: "info", text: "Any model available in Proxy Max can be used — not just Qwen models. Select from Qwen, Claude, Gemini, GPT, and more." },
      { type: "warning", text: "Config path: Linux/macOS ~/.qwen/settings.json • Windows %USERPROFILE%\\.qwen\\settings.json" },
      { type: "error", text: "Qwen OAuth free tier was discontinued on 2026-04-15. Use Proxy Max with alicode/openrouter/anthropic/gemini providers instead." },
    ],
  ...
  "deepseek-tui": {
    ...
    notes: [
      { type: "info", text: "DeepSeek TUI uses ~/.deepseek/config.toml for configuration. Proxy Max will update the provider to 'openai' mode with your base_url, api_key, and model." },
  ...
  jcode: {
    ...
    notes: [
      ...
      {
        type: "info",
        text: "Configure proxy-max as an OpenAI-compatible provider to route all jcode requests through proxy-max's optimization layer."
      },
      {
        type: "warning",
  ...
  "grok-build": {
    ...
    notes: [
      {
        type: "info",
        text: "Grok Build uses ~/.grok/config.toml. Proxy Max writes a [model.proxy-max] custom model and sets it as the default.",
      },
      {
        type: "info",
        text: "After Apply, run grok (or /model proxy-max) to use the routed model. Switch back anytime with /model grok-build.",
      },
      {
        type: "warning",
