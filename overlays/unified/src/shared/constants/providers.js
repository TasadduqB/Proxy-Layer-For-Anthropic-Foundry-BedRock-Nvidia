...
function buildProviderEntry(r) {
  ...
  return {
    ...
    ...(Array.isArray(r.aliases) && r.aliases.length ? { aliases: [...r.aliases] } : {}),
...
export const ALIAS_TO_ID = Object.values(AI_PROVIDERS).reduce((acc, p) => {
  // Match getProviderByAlias's first-match precedence. This matters for the
  // legacy hidden `mmf` compatibility entry: the public `mimo-free` provider
  // intentionally owns `mmf` as its UI alias and appears first.
  const add = (key) => {
    if (key && !Object.prototype.hasOwnProperty.call(acc, key)) acc[key] = p.id;
  };
  add(p.id);
  add(p.alias);
