...
function generateRootCA() {
  ...
  if (exists && !isCertExpired(ROOT_CA_CERT_PATH)) {
    try { fs.chmodSync(ROOT_CA_KEY_PATH, 0o600); } catch {}
  ...
  try { fs.chmodSync(MITM_DIR, 0o700); } catch {}
  ...
  const attrs = [
    { name: "commonName", value: "Proxy Max MITM Root CA" },
    { name: "organizationName", value: "Proxy Max" },
  ...
  const keyTemp = `${ROOT_CA_KEY_PATH}.${process.pid}.tmp`;
  const certTemp = `${ROOT_CA_CERT_PATH}.${process.pid}.tmp`;
  try {
    fs.writeFileSync(keyTemp, privateKeyPem, { mode: 0o600, flag: "wx" });
    fs.writeFileSync(certTemp, certPem, { mode: 0o644, flag: "wx" });
    fs.renameSync(keyTemp, ROOT_CA_KEY_PATH);
    fs.renameSync(certTemp, ROOT_CA_CERT_PATH);
    try { fs.chmodSync(ROOT_CA_KEY_PATH, 0o600); } catch {}
  } finally {
    try { fs.unlinkSync(keyTemp); } catch {}
    try { fs.unlinkSync(certTemp); } catch {}
  }
...
function generateLeafCert(domain, rootCA) {
  const safeDomain = domainToASCII(String(domain || "").trim().replace(/\.$/, "")).toLowerCase();
  if (!safeDomain || safeDomain.length > 253 || !/^[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?$/.test(safeDomain) || safeDomain.split(".").some((label) => !label || label.length > 63)) {
    throw new Error("Invalid certificate domain");
  }
  ...
  cert.setSubject([
    { name: "commonName", value: safeDomain }
  ]);
  ...
  cert.setExtensions([
    ...
    },
    {
      name: "subjectAltName",
      altNames: [
        { type: 2, value: safeDomain }, // DNS
        { type: 2, value: `*.${safeDomain}` } // Wildcard
      ]
    }
  ]);
