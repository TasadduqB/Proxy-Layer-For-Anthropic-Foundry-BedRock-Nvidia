...
const { IS_DEV, TARGET_HOSTS, URL_PATTERNS, MODEL_SYNONYMS, MODEL_PATTERNS, MODEL_NO_MAP, getToolForHost } = require("./config");
...
const INSTANCE_TOKEN = String(process.env.MITM_INSTANCE_TOKEN || "");
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("data", chunk => {
      ...
      if (size > MAX_REQUEST_BODY_BYTES) {
        exceeded = true;
        const error = new Error("MITM request body exceeds 64 MiB");
...
const server = https.createServer(sslOptions, async (req, res) => {
  try {
    if (req.url === "/_mitm_health") {
      const supplied = String(req.headers["x-9r-mitm-instance"] || "");
      const valid = INSTANCE_TOKEN && supplied.length === INSTANCE_TOKEN.length &&
        require("crypto").timingSafeEqual(Buffer.from(supplied), Buffer.from(INSTANCE_TOKEN));
      if (!valid) {
    ...
    // Anti-loop: skip requests from Proxy Max
    ...
    const status = e.code === "BODY_TOO_LARGE" ? 413 : 500;
    if (!res.headersSent) res.writeHead(status, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ error: { message: status === 413 ? "Request body too large" : "MITM request failed", type: "mitm_error" } }));
  }
});

// A stale process is handled by the authenticated manager before spawn. The
// server itself never kills an arbitrary listener merely because it owns 443.
server.listen(LOCAL_PORT, LOOPBACK_HOST, () => log(`🚀 Server ready on ${LOOPBACK_HOST}:${LOCAL_PORT}`));
