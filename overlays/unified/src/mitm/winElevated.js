const { runFile, runFileSync, safeErrorMessage } = require("./process.js");
...
function isAdmin() {
  if (IS_WIN) {
    try {
      runFileSync("net.exe", ["session"], { stdio: "ignore", timeoutMs: 3000 });
      return true;
    } catch {
      return false;
    }
...
function runElevatedPowerShell(script) {
  ...
  if (isAdmin()) {
    ...
    ], { timeoutMs: 120_000, maxOutputBytes: 256 * 1024 }).then((result) => result.stdout);
  }
  ...
  return runFile("powershell.exe", [
    "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-Command", wrapper,
  ], { timeoutMs: 120_000, maxOutputBytes: 256 * 1024 })
    .then((result) => result.stdout)
    .catch((error) => {
      const msg = safeErrorMessage(error);
      if (/canceled by the user|operation was canceled/i.test(msg)) throw new Error("User canceled UAC prompt");
      throw new Error(msg);
    });
}
