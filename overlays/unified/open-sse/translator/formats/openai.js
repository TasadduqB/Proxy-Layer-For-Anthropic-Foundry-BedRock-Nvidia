...
export function filterToOpenAIFormat(body, opts = {}) {
  ...
  function normalizeToolMessage(msg) {
    const { is_error: isError, ...normalized } = msg;
    if (isError !== true) return normalized;

    const errorMarker = "[Tool execution failed]";
    if (typeof normalized.content === "string") {
      if (!normalized.content.startsWith(errorMarker)) {
        normalized.content = `${errorMarker}\n${normalized.content}`;
      }
    } else if (Array.isArray(normalized.content)) {
      normalized.content = [
        { type: OPENAI_BLOCK.TEXT, text: errorMarker },
        ...normalized.content,
      ];
    } else {
      normalized.content = `${errorMarker}\n${JSON.stringify(normalized.content ?? null)}`;
    }
    return normalized;
  }
  ...
  body.messages = body.messages.map(msg => {
    ...
    // OpenAI tool messages do not accept Anthropic's is_error extension.
    // Preserve a true error as model-visible content; always remove the wire field.
    if (msg.role === ROLE.TOOL) return normalizeToolMessage(msg);
