...
import {
  ...
  documentToResponsesFile,
  normalizeOpenAIFile,
} from "../concerns/document.js";
...
export function openaiResponsesToOpenAIRequest(model, body, stream, credentials) {
  ...
  for (const item of inputItems) {
    ...
    if (itemType === RESPONSES_ITEM.MESSAGE) {
      ...
      const content = Array.isArray(item.content)
        ? item.content.map(c => {
          ...
          if (c.type === RESPONSES_ITEM.INPUT_IMAGE) {
            ...
            // A Responses file_id is not a URL. Preserve it as a Chat file
            // block so later translators can handle it without issuing an
            // invalid fetch for a literal such as "file-abc".
            if (typeof c.file_id === "string" && c.file_id) {
              return { type: OPENAI_BLOCK.FILE, file: { file_id: c.file_id } };
            }
            return null;
          }
          if (c.type === RESPONSES_ITEM.INPUT_FILE) {
            return documentToOpenAIFile(normalizeOpenAIFile(c));
          }
          return c;
        }).filter(Boolean)
        : item.content;
      ...
      const toolName = typeof item.name === "string" && item.name.trim() ? item.name.trim() : "_unknown";
...
export function openaiToOpenAIResponsesRequest(model, body, stream, credentials) {
      ...
      const content = typeof msg.content === "string"
        ...
        : Array.isArray(msg.content)
          ? msg.content.map(c => {
            ...
            if (c.type === OPENAI_BLOCK.FILE) {
              return documentToResponsesFile(normalizeOpenAIFile(c.file));
            }
            if (c.type === RESPONSES_ITEM.INPUT_FILE) return c;
  ...
  // Responses endpoints use max_output_tokens. Normalize both Chat spellings so
  // Azure/OpenAI Responses never receive the unsupported max_tokens fields.
