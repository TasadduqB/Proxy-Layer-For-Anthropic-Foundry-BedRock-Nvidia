    ...
    for (const part of content) {
      if (typeof part === "string") {
        ...
        if (part.type === OPENAI_BLOCK.TEXT && typeof part.text === "string") {
          ...
          if (url) blocks.push({ type: OPENAI_BLOCK.TEXT, text: `[image_url: ${url}]` });
        } else if (part.type === OPENAI_BLOCK.IMAGE) {
          const source = part.source || {};
          const value = source.type === "base64"
            ? `data:${source.media_type || "application/octet-stream"};base64,${source.data || ""}`
            : source.url;
          if (value) blocks.push({ type: OPENAI_BLOCK.TEXT, text: `[image: ${value}]` });
