...
import { documentToClaudeBlock, normalizeOpenAIFile } from "../concerns/document.js";
...
Respond ONLY with the JSON object, no other text.`);
  ...
  const shouldInjectClaudeCodePrompt = provider == null || provider === "claude" || provider === "anthropic";
  const claudeCodePrompt = { type: CLAUDE_BLOCK.TEXT, text: CLAUDE_SYSTEM_PROMPT };

  if (systemParts.length > 0) {
    const systemText = systemParts.join("\n");
    result.system = [
      ...(shouldInjectClaudeCodePrompt ? [claudeCodePrompt] : []),
      { type: CLAUDE_BLOCK.TEXT, text: systemText, cache_control: { type: "ephemeral", ttl: "1h" } }
    ];
  } else if (shouldInjectClaudeCodePrompt) {
...
function convertOpenAIToolResultContent(content) {
  if (!Array.isArray(content)) return content;
  const blocks = [];
  for (const part of content) {
    if (typeof part === "string") {
      blocks.push({ type: CLAUDE_BLOCK.TEXT, text: part });
      ...
      const document = documentToClaudeBlock(normalizeOpenAIFile(part.file));
      if (document) blocks.push(document);
    } else if (part != null) {
      blocks.push({ type: CLAUDE_BLOCK.TEXT, text: JSON.stringify(part) });
    }
  }
...
function getContentBlocksFromMessage(msg, toolNameMap = new Map()) {
  ...
  if (msg.role === ROLE.TOOL) {
    const content = convertOpenAIToolResultContent(msg.content);
    blocks.push({
      type: CLAUDE_BLOCK.TOOL_RESULT,
      tool_use_id: msg.tool_call_id,
      content,
      ...(msg.is_error !== undefined && { is_error: msg.is_error === true }),
    });
    ...
    if (typeof msg.content === "string") {
      ...
      for (const part of msg.content) {
        if (part.type === OPENAI_BLOCK.TEXT && part.text) {
          ...
          const document = documentToClaudeBlock(normalizeOpenAIFile(part.file));
          if (document) blocks.push(document);
        } else if (part.type === OPENAI_BLOCK.INPUT_AUDIO && part.input_audio?.data) {
          const format = String(part.input_audio.format || "wav").toLowerCase();
          const mediaType = format === "mp3" ? "audio/mpeg" : `audio/${format}`;
          blocks.push({
            type: "audio",
            source: { type: "base64", media_type: mediaType, data: part.input_audio.data },
          });
        } else if (part.type === OPENAI_BLOCK.AUDIO_URL) {
          const url = typeof part.audio_url === "string" ? part.audio_url : part.audio_url?.url;
          if (url) blocks.push({ type: "audio", source: { type: "url", url } });
        }
      }
    }
  } else if (msg.role === ROLE.ASSISTANT) {
    // OpenAI-compatible reasoning models commonly place prior assistant
    // thinking in reasoning_content. Preserve it as a Claude thinking block
    // before the visible assistant content instead of silently discarding it.
    ...
    if (Array.isArray(msg.reasoning_details)) {
      for (const detail of msg.reasoning_details) {
        if (detail?.type === CLAUDE_BLOCK.REDACTED_THINKING && detail.data) {
          blocks.push({ type: CLAUDE_BLOCK.REDACTED_THINKING, data: detail.data });
        }
      }
    }
