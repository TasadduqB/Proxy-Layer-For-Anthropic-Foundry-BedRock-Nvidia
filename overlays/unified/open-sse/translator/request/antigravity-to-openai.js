...
import { documentToOpenAIFile, isPdfDocument, normalizeGeminiDocumentPart } from "../concerns/document.js";
...
function convertContent(content) {
  ...
  for (const part of content.parts) {
    ...
    // Inline/file data. PDFs remain file blocks through the OpenAI pivot;
    // other inline media retains the prior image representation.
    const media = normalizeGeminiDocumentPart(part);
    if (media && isPdfDocument(media)) {
      const file = documentToOpenAIFile(media);
      if (file) textParts.push(file);
    } else if (media?.type === "base64") {
      textParts.push({
        type: OPENAI_BLOCK.IMAGE_URL,
        image_url: { url: encodeDataUri(media.mimeType, media.data) }
      });
    } else if (media?.type === "url") {
      textParts.push({ type: OPENAI_BLOCK.IMAGE_URL, image_url: { url: media.url } });
    }
