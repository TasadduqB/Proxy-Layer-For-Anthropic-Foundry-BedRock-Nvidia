...
function applyFormat(fmt, body, cfg, caps) {
  ...
  switch (fmt) {
    case "openai": {
      ...
      // "auto" (from a client's own budget math going negative/invalid — see
      // extractThinking's budget<=0 fallback) is valid on real OpenAI, but many
      // OpenAI-compatible backends (e.g. NVIDIA NIM's vllm) reject it outright
      // ("Input should be 'low', 'medium' or 'high'"), failing every model in
      // a combo identically. Send a concrete level every such backend accepts.
      if (level) body.reasoning_effort = level === "max" ? "xhigh" : (level === "auto" ? "medium" : level);
      break;
    }
