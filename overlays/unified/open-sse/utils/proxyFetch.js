...
const requestProxyContext = new AsyncLocalStorage();

const PROXY_OPTION_KEYS = [
  "enabled",
  "url",
  "noProxy",
...
function copyProxyOptions(value) {
  if (!value || typeof value !== "object") return null;
  const copied = {};
  for (const key of PROXY_OPTION_KEYS) {
    if (value[key] !== undefined) copied[key] = value[key];
  }
  return copied;
}

/** Build a request-scoped proxy policy from a resolved connection. */
export function proxyOptionsFromCredentials(credentials) {
  ...
  return {
    ...
    vercelRelayUrl: normalizeString(data.vercelRelayUrl),
    strictProxy: data.strictProxy === true,
  };
}

/**
 * Isolate proxy policy per asynchronous request. Raw global fetch calls made by
 * nested provider adapters inherit this context without leaking across requests.
 */
export function runWithProxyOptions(proxyOptions, callback) {
  if (typeof callback !== "function") {
    throw new TypeError("runWithProxyOptions requires a callback");
  }
  return requestProxyContext.run(copyProxyOptions(proxyOptions), callback);
}

export function runWithCredentialsProxy(credentials, callback) {
  return runWithProxyOptions(proxyOptionsFromCredentials(credentials), callback);
}
...
function safeProxyError(error) {
  return String(error?.message || error || "proxy transport failed")
...
export async function proxyAwareFetch(url, options = {}, proxyOptions = null) {
  proxyOptions = copyProxyOptions(proxyOptions ?? requestProxyContext.getStore());
  const targetUrl = typeof url === "string" ? url : url.toString();
  // Dependency injection keeps offline verification hermetic without replacing
  // the process-wide fetch that this module deliberately wraps.
  const directFetch = typeof proxyOptions?.fetchFn === "function"
    ? proxyOptions.fetchFn
    : originalFetch;
  ...
  const configuredConnectionProxy = normalizeString(
    proxyOptions?.url ?? proxyOptions?.connectionProxyUrl,
  );
  if (
    proxyOptions?.strictProxy === true &&
    connectionProxyEnabled &&
    !configuredConnectionProxy
  ) {
    throw new Error(
      "[ProxyFetch] Proxy required but no connection proxy URL is configured",
    );
  }
    ...
    if (proxyUrl) {
      ...
      try {
        ...
        if (proxyOptions?.strictProxy === true) {
          throw new Error(`[ProxyFetch] Proxy required but failed (strictProxy=true): ${safeProxyError(proxyError)}`);
        }
        console.warn(`[ProxyFetch] Proxy failed, falling back to direct bypass: ${safeProxyError(proxyError)}`);
      }
    }
  ...
  if (proxyUrl) {
    try {
      ...
      console.warn(`[ProxyFetch] Proxy failed, falling back to direct: ${safeProxyError(proxyError)}`);
