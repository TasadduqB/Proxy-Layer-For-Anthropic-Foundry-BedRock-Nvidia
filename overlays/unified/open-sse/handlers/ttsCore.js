 ...
 */
async function handleTtsCoreImpl({ provider, model, input, credentials, responseFormat = "mp3", language }) {
  if (!input?.trim()) {
...
export function handleTtsCore(options) {
  return runWithCredentialsProxy(options?.credentials, () =>
    handleTtsCoreImpl(options),
  );
}
