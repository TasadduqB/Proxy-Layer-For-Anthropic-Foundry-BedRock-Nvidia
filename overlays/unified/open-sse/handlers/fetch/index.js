 ...
 */
async function handleFetchCoreImpl({ url, format, maxCharacters, provider, providerConfig, credentials, log }) {
...
export function handleFetchCore(options) {
  return runWithCredentialsProxy(options?.credentials, () =>
    handleFetchCoreImpl(options),
  );
}
