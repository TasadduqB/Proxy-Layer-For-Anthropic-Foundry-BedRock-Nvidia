...
import { prefetchRemoteDocuments, prefetchRemoteImages } from "../translator/concerns/prefetch.js";
import { extractThinking } from "../translator/concerns/thinkingUnified.js";
import { resolveSessionId } from "../utils/sessionManager.js";
import { runWithCredentialsProxy } from "../utils/proxyFetch.js";
import { injectAuthorizedCredentialsGuardrail } from "../guardrails/authorizedCredentials.js";
...
function refreshHeadroomAfterSnapshot(body, diagnostics) {
  if (!diagnostics?.after) return;
  ...
  diagnostics.after = {
    ...
    ...(messages ? { messageBytes: jsonByteLength(messages) } : {}),
  };
}
...
async function handleChatCoreImpl({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking }) {
  ...
  // Preserve Proxy-Max's existing always-on credential handling instruction in
  // every native client shape before translation or provider passthrough.
  injectAuthorizedCredentialsGuardrail(body, sourceFormat);

  const alias = PROVIDER_ID_TO_ALIAS[provider] || provider;
  const modelTargetFormat = getModelTargetFormat(alias, model);
  // Multi-endpoint providers: pick transport matching sourceFormat → zero translation
  const runtimeTransport = resolveTransport(provider, sourceFormat, credentials);
  const targetFormat = runtimeTransport?.forceTargetFormat
  ...
  const providerRequiresStreaming = PROVIDERS[provider]?.forceStream === true || runtimeTransport?.forceStream === true;
  ...
  if (!passthrough) {
    ...
    try {
      const n = await prefetchRemoteDocuments(body, sourceFormat, targetFormat, { signal: undefined });
      if (n > 0) log?.debug?.("MODALITY", `prefetched ${n} remote PDF(s) for ${targetFormat}`);
    } catch {
      // Never silently drop a PDF or forward an unverified URL to a protocol
      // that requires inline bytes. Keep the response free of URL/DNS details.
      return createErrorResult(HTTP_STATUS.BAD_REQUEST, "Remote PDF could not be fetched safely");
    }
  }
  ...
  if (headroomStats) {
    // A compressor controls its returned message array and may omit system or
    // developer messages. Reassert the mandatory guardrail before measuring
    // and dispatching the transformed request.
    injectAuthorizedCredentialsGuardrail(translatedBody, finalFormat);
    refreshHeadroomAfterSnapshot(translatedBody, headroomDiagnostics);
  }
  ...
  // Keep the always-on credential guardrail invariant across every optional
  // body replacement above (Headroom, Caveman, Ponytail, and PXPIPE).
  injectAuthorizedCredentialsGuardrail(translatedBody, finalFormat);
  ...
  try {
    ...
    const clientSafeError = typeof executor.sanitizeClientError === "function"
      ? executor.sanitizeClientError(error)
      : error;
    trackPendingRequest(model, provider, connectionId, false, true);
    ...
    saveRequestDetail(buildRequestDetail({
      ...
      response: { error: clientSafeError.message || String(clientSafeError), status: error.name === "AbortError" ? 499 : 502, thinking: null },
    ...
    const errMsg = formatProviderError(clientSafeError, provider, model, HTTP_STATUS.BAD_GATEWAY);
    if (log?.errorLine) {
      log.errorLine(reqTag, "✗", `ERROR 502 · ${provider}/${model} · ${Date.now() - requestStartTime}ms\n    ${errMsg}${clientSafeError.stack ? `\n    ${clientSafeError.stack}` : ""}`);
    }
  ...
  if (!executor.noAuth && (providerResponse.status === HTTP_STATUS.UNAUTHORIZED || providerResponse.status === HTTP_STATUS.FORBIDDEN)) {
    try {
      // A 401/403 refresh is part of the same account-bound request and must
      // obey its proxy-pool policy too. Without the third argument, refreshes
      // silently bypass strict pools even though the executors support it.
...
export function handleChatCore(options) {
  return runWithCredentialsProxy(options?.credentials, () =>
    handleChatCoreImpl(options),
  );
}
