...
function isTextOnlyContent(content, allowEmpty = false) {
  if (typeof content === "string") return allowEmpty || content.length > 0;
  if (!Array.isArray(content)) return false;
  return content.every((part) => part?.type === "text" && typeof part.text === "string");
}

function isValidAgentToolCall(toolCall) {
  if (!toolCall || typeof toolCall !== "object") return false;
  const fn = toolCall.function;
  return (!toolCall.type || toolCall.type === "function")
    && typeof fn?.name === "string"
    && fn.name.length > 0;
}

/**
 * Whether an OpenAI-style request can be represented by Cursor AgentService.
 * AgentService supports text, declared MCP tools, assistant tool calls, and
 * tool results. Binary media remains on the legacy Cursor transport.
 */
export function isAgentCapableRequest(body) {
  if (!Array.isArray(body?.messages) || body.messages.length === 0) return false;
  return body.messages.every((message) => {
    if (!message || typeof message !== "object") return false;
    if (!["system", "developer", "user", "assistant", "tool"].includes(message.role)) return false;

    if (message.role === "assistant" && Array.isArray(message.tool_calls)) {
      if (!message.tool_calls.every(isValidAgentToolCall)) return false;
      return message.content == null || isTextOnlyContent(message.content, true);
    }
    if (message.role === "tool") {
      return typeof message.tool_call_id === "string"
        && message.tool_call_id.length > 0
        && isTextOnlyContent(message.content, true);
    }
    return isTextOnlyContent(message.content, true);
  });
}
...
function historyTextFromMessage(message) {
  const sections = [];
  ...
  if (message?.role === "assistant" && Array.isArray(message.tool_calls)) {
    for (const toolCall of message.tool_calls) {
      if (!isValidAgentToolCall(toolCall)) continue;
      sections.push(JSON.stringify({
        type: "tool_call",
        id: toolCall.id || "",
        name: toolCall.function.name,
        arguments: formatToolArguments(toolCall.function.arguments),
      }));
    }
...
function encodeHistoryMessage(message) {
  const content = historyTextFromMessage(message);
...
export function buildAgentRunFrame(messages, model, tools = []) {
  ...
  const system = safeMessages
    .filter((message) => message?.role === "system" || message?.role === "developer")
    .map((message) => textFromContent(message.content))
    .filter(Boolean)
    .join("\n\n");
  const chatMessages = safeMessages.filter((message) =>
    message?.role !== "system" && message?.role !== "developer"
  );
  // A trailing user message is the active action. If the request ends in an
  // assistant/tool result, replay the complete history and send a resume turn.
  const currentIndex = chatMessages.at(-1)?.role === "user" ? chatMessages.length - 1 : -1;
  const current = currentIndex >= 0 ? chatMessages[currentIndex] : null;
  const historyMessages = currentIndex >= 0 ? chatMessages.slice(0, currentIndex) : chatMessages;
  ...
  const runRequest = concatBuffers(
    ...
    ...(mcpTools.length > 0 ? [agentMessage(4, mcpTools)] : []),
...
function createRequestContextResponse(execId = 0) {
  // AgentService asks every run for client context. proxy-max has no IDE file
  ...
  const execClientMessage = concatBuffers(
    encodeField(1, PROTOBUF_VARINT, Number(execId) || 0),
      ...
      try {
        while (!finished) {
          ...
          pending = decodeAgentFrames(pending, (payload) => {
            ...
            // Return an empty context; proxy-max is not coupled to an editor.
            if (serverMessage.has(2)) {
              const execRequest = decodeMessage(serverMessage.get(2)[0].value);
              const execId = execRequest.get(1)?.[0]?.value || 0;
              if (execRequest.has(10)) {
                session.write(createRequestContextResponse(execId));
              } else if (execRequest.has(11)) {
                try {
                  const mcpArgs = decodeMcpArgs(execRequest.get(11)[0].value);
                  const name = mcpArgs.toolName || mcpArgs.name || "tool";
                  const id = mcpArgs.toolCallId || `call_${crypto.randomUUID()}`;
                  let args = "{}";
                  try { args = JSON.stringify(mcpArgs.args ?? {}); } catch {}
                  finished = true;
                  onEvent({
                    type: "tool_call",
                    value: {
                  ...
                  onEvent({
                    type: "error",
                    value: `Cursor AgentService returned invalid MCP arguments: ${error.message}`,
                  });
                  onEvent({ type: "done", finishReason: "stop" });
                }
              } else {
        ...
        if (!finished) onEvent({ type: "done", finishReason: "stop" });
      }
    };
    ...
    if (stream === false) {
      ...
      return {
        response: new Response(JSON.stringify({
          ...
          usage: estimateUsage(
            body,
            content.length + toolCalls.reduce((total, call) => total + call.function.arguments.length, 0),
            FORMATS.OPENAI,
          ),
        }), { headers: { "Content-Type": "application/json" } }),
        url,
        ...
        consume((event) => {
          if (event.type === "text") {
            ...
            controller.enqueue(encoder.encode(chatChunkSse({
              ...
              delta: {
                role: "assistant",
                tool_calls: [{ index: 0, ...event.value }],
              },
            })));
