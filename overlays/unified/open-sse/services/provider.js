...
// to avoid lossy translation. Azure is connection-scoped: Responses connections
// always use the Responses wire format and streaming even when the client sent Chat.
export function resolveTransport(provider, sourceFormat, credentials = null) {
  const azureApiType = typeof credentials?.providerSpecificData?.apiType === "string"
    ? credentials.providerSpecificData.apiType.trim().toLowerCase()
    : "";
  if (provider === "azure" && azureApiType === "responses") {
    return {
