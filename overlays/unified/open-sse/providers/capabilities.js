...
// A model family being able to understand a PDF is not sufficient: the
// provider transport must also preserve a document block. Keep this list
// deliberately conservative so combo routing never promotes a connection that
// will silently flatten or discard the file (Kiro/Cursor/other CLI protocols).
const PDF_PRESERVING_PROVIDERS = new Set([
  "anthropic", "claude", "cc", "bedrock",
  "gemini", "gemini-cli", "gc", "vertex", "vx", "antigravity", "ag",
  "openai", "openrouter",
]);

const PDF_MODEL_FAMILY = /(?:^|[/_.-])(?:claude|gemini|gpt-(?:4o|4\.1|5)|o[134](?:$|[-_.]))/i;

function providerPreservesPdf(provider) {
  if (!provider) return true;
  const normalized = String(provider).toLowerCase();
  return PDF_PRESERVING_PROVIDERS.has(normalized) || normalized.startsWith("anthropic-compatible-");
}

function finalizeCapabilities(provider, model, caps) {
  const modelCanReadPdf = caps.pdf === true || (caps.vision === true && PDF_MODEL_FAMILY.test(String(model || "")));
  return { ...caps, pdf: modelCanReadPdf && providerPreservesPdf(provider) };
}
...
export function getCapabilitiesForModel(provider, model) {
  ...
  if (provider) {
    ...
    if (providerCaps?.[model]) return finalizeCapabilities(provider, model, { ...DEFAULT_CAPABILITIES, ...providerCaps[model] });
    if (providerCaps?.[baseModel]) return finalizeCapabilities(provider, model, { ...DEFAULT_CAPABILITIES, ...providerCaps[baseModel] });
  }

  // 2. Canonical exact
  if (MODEL_CAPABILITIES[baseModel]) return finalizeCapabilities(provider, model, { ...DEFAULT_CAPABILITIES, ...MODEL_CAPABILITIES[baseModel] });
  if (MODEL_CAPABILITIES[model]) return finalizeCapabilities(provider, model, { ...DEFAULT_CAPABILITIES, ...MODEL_CAPABILITIES[model] });

  // 3. Pattern match (first match wins)
  for (const { pattern, caps } of PATTERN_CAPABILITIES) {
    if (matchPattern(pattern, baseModel) || matchPattern(pattern, model)) {
      return finalizeCapabilities(provider, model, { ...DEFAULT_CAPABILITIES, ...caps });
    }
  }

  // 4. Floor
  return finalizeCapabilities(provider, model, { ...DEFAULT_CAPABILITIES });
}
