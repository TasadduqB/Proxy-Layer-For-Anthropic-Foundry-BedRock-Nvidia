...
describe("prefetchRemoteImages", () => {
  ...
  it("openai source -> kiro target: reaches Kiro's inline images field", async () => {
    const body = { messages: [{ role: "user", content: [
      { type: "text", text: "inspect" },
      { type: "image_url", image_url: { url: "https://x/a.png" } },
    ] }] };

    const n = await prefetchRemoteImages(body, FORMATS.OPENAI, FORMATS.KIRO);
    const translated = translateRequest(FORMATS.OPENAI, FORMATS.KIRO, "m", body, true, null, "kiro");
    const images = translated.conversationState?.currentMessage?.userInputMessage?.images || [];

    expect(n).toBe(1);
    expect(images).toEqual([{ format: "png", source: { bytes: "QUJD" } }]);
  });
