...
describe("dashboard guard local-only access", () => {
  ...
  it("rejects a remote local-only route even with a valid CLI token", async () => {
    const response = await proxy(request("/api/mcp/filesystem/sse", {
      host: "router.example.com",
  ...
  it("allows a local-only route with a valid CLI token on loopback", async () => {
    const response = await proxy(request("/api/mcp/filesystem/sse", {
      host: "localhost:20128",
      "x-9r-real-ip": "127.0.0.1",
      "x-9r-cli-token": "cli-token",
...
describe("password-free mode remains loopback-only", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.getSettings.mockResolvedValue({ requireLogin: false });
    mocks.validateApiKey.mockResolvedValue(false);
  ...
  it("allows a local management request when login is disabled", async () => {
  ...
  it("rejects remote management when login is disabled", async () => {
    const response = await proxy(request("/api/settings", {
      host: "router.example.com",
  ...
  it("does not expose the require-login settings route remotely", async () => {
    const response = await proxy(request("/api/settings/require-login", {
      host: "router.example.com",
      "x-9r-real-ip": "10.1.2.3",
    }));
    expect(response.status).toBe(401);
  ...
  it("keeps OIDC configuration testing protected while login endpoints remain public", async () => {
    const remoteHeaders = {
      host: "router.example.com",
      "x-9r-real-ip": "10.1.2.3",
    };
    expect((await proxy(request("/api/auth/oidc/test", remoteHeaders))).status).toBe(401);
    expect(await proxy(request("/api/auth/oidc/start", remoteHeaders))).toBe(mocks.nextResponse);
    expect(await proxy(request("/api/auth/oidc/callback", remoteHeaders))).toBe(mocks.nextResponse);
  });

  it("returns a bounded denial instead of redirect-looping a remote dashboard", async () => {
    const response = await proxy(request("/dashboard", {
      host: "router.example.com",
      "x-9r-real-ip": "10.1.2.3",
    }));
    expect(response.status).toBe(403);
    expect(response.body.error).toMatch(/requires authentication/);
  });
});
