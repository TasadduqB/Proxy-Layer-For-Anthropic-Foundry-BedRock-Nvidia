...
// qwen/iflow retain informational handlers for legacy hidden connections even
// though the registry does not advertise quota UI for them.
const SUPPORTED = [...new Set([...USAGE_SUPPORTED_PROVIDERS, "qwen", "iflow"])];
...
describe("usage dispatch", () => {
  ...
  it("forwards provider-specific context to informational usage handlers", async () => {
    const { getUsageForProvider } = await load();
    const res = await getUsageForProvider({
      provider: "ollama",
      accessToken: "t",
      providerSpecificData: { plan: "Team" },
    });
