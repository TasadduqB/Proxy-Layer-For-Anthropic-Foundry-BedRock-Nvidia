...
import {
  normalizeRepeatedToolInput,
  openaiToClaudeWithEditRepair,
} from "../../open-sse/translator/response/openai-to-claude-edit-repair.js";
import { kiroToClaudeWithInputRepair } from "../../open-sse/translator/response/kiro-to-claude-input-repair.js";
...
describe("openaiToClaudeResponse", () => {
  ...
  it("collapses repeated complete Skill arguments from snapshot-style streams", () => {
    const state = { toolCalls: new Map() };
    const args = JSON.stringify({ skill: "review", args: "177" });

    openaiToClaudeWithEditRepair({
      id: "chatcmpl-skill-review",
      model: "gpt-test",
      choices: [{
        delta: {
          tool_calls: [{
    ...
    const result = openaiToClaudeWithEditRepair({
      id: "chatcmpl-skill-review",
      model: "gpt-test",
      choices: [{ delta: {}, finish_reason: "tool_calls" }],
    }, state);
    ...
    expect(inputDelta.delta.partial_json).toBe(args);
    expect(JSON.parse(inputDelta.delta.partial_json)).toEqual({ skill: "review", args: "177" });
  });

  it("keeps the final object from evolving full argument snapshots", () => {
    const invalid = '{"skill":"review","args":"177"}{"skill":"review","args":"178"}';
    expect(normalizeRepeatedToolInput(invalid)).toEqual({
      value: '{"skill":"review","args":"178"}',
      repaired: true,
      copies: 2,
    });
  });
  ...
  it.each([
    {
      tool: "Bash",
      snapshots: [
        { command: "gh pr view 177" },
        { command: "gh pr view 177 --json reviews", description: "Inspect open reviews" },
      ],
    },
    {
      tool: "Read",
      snapshots: [
        { file_path: "/tmp/agent_session_store.py" },
        { file_path: "/tmp/agent_session_store.py", offset: 0, limit: 200 },
      ],
    },
  ])("repairs evolving $tool snapshots from snapshot-style streams", ({ tool, snapshots }) => {
    const state = { toolCalls: new Map() };
    snapshots.forEach((input, index) => {
      openaiToClaudeWithEditRepair({
        id: `chatcmpl-${tool.toLowerCase()}-snapshot`,
        model: "gpt-test",
        choices: [{
          delta: {
            tool_calls: [{
              index: 0,
              ...(index === 0 ? { id: `toolu_${tool.toLowerCase()}` } : {}),
              function: { ...(index === 0 ? { name: tool } : {}), arguments: JSON.stringify(input) },
            }],
          },
        }],
      }, state);
    ...
    expect(JSON.parse(inputDelta.delta.partial_json)).toEqual(snapshots.at(-1));
  });

  it("repairs repeated Skill snapshots on the direct Kiro response edge", () => {
    const state = { toolCalls: new Map() };
    const args = JSON.stringify({ skill: "review", args: "177" });
    kiroToClaudeWithInputRepair({
      id: "chatcmpl-kiro-skill",
      model: "kiro-test",
      choices: [{ delta: { tool_calls: [{ index: 0, id: "toolu_kiro_skill", function: { name: "Skill", arguments: args } }] } }],
    }, state);
    kiroToClaudeWithInputRepair({
      id: "chatcmpl-kiro-skill",
      model: "kiro-test",
    ...
    const result = kiroToClaudeWithInputRepair({
      id: "chatcmpl-kiro-skill",
      model: "kiro-test",
      choices: [{ delta: {}, finish_reason: "tool_calls" }],
    }, state);
