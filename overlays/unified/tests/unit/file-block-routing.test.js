...
describe("file/document block support", () => {
  ...
  it("gemini: preserves a remote PDF as fileData until safe prefetch", () => {
    const parts = convertOpenAIContentToParts([
      { type: "file", file: { filename: "d.pdf", file_data: "https://x/d.pdf" } },
    ]);
    expect(parts).toContainEqual({
      fileData: { mimeType: "application/pdf", fileUri: "https://x/d.pdf" },
    });
  });
  ...
  it("claude: preserves a remote PDF URL as a document source", () => {
    const out = openaiToClaudeRequest("claude-x", {
      messages: [{ role: "user", content: [
        { type: "file", file: { filename: "d.pdf", file_data: "https://files.example/d.pdf" } },
      ] }],
    }, false);
    expect(out.messages[0].content).toContainEqual({
      type: "document",
      source: { type: "url", url: "https://files.example/d.pdf" },
    });
  });
});
