...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("proxy-max");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "proxy-max-general-purpose",
      explore: "proxy-max-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.proxy-max\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.proxy-max-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.proxy-max-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# proxy-max-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.proxy-max-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("proxy-max-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.proxy-max-");
    expect(reset).not.toContain("proxy-max-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("proxy-max-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("proxy-max-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("proxy-max-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("proxy-max-explore");
    expect(getGrokSubagentSlot("plan")).toBe("proxy-max-plan");
