...
describe("openai→claude: image_url.detail is dropped (docs 11 §4)", () => {
  ...
  it("preserves input_audio for compatible Claude-style endpoints", () => {
    const body = {
      model: "claude-sonnet-4-6",
    ...
    expect(blocks.find((b) => b.type === "audio")).toEqual({
      type: "audio",
      source: { type: "base64", media_type: "audio/wav", data: "ZZZ" },
    });
  });
});
