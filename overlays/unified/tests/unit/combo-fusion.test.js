...
describe("fusion combo", () => {
  ...
  it("fans out to the panel then routes a synthesis turn to the judge", async () => {
    ...
    // Panel calls keep an explicitly streaming client's wire format while
    // stripping tools. This lets a sole survivor be returned without rebilling.
  ...
  it("preserves the sole survivor response body and headers without rebilling it", async () => {
    const survivor = new Response(JSON.stringify({ choices: [{ message: { content: "only answer" } }] }), {
      status: 200,
      headers: { "Content-Type": "application/json", "X-Upstream-Request": "req-1" },
    });
    const failed = new Response(JSON.stringify({ error: { message: "boom" } }), {
      status: 503,
      headers: { "Content-Type": "application/json" },
    });
    const handleSingleModel = vi.fn(async (_body, model) => model === "p/ok" ? survivor : failed);
    ...
    expect(res.headers.get("x-upstream-request")).toBe("req-1");
    await expect(res.json()).resolves.toEqual({
      choices: [{ message: { content: "only answer" } }],
    });
  });

  it("preserves a sole survivor SSE stream without rebilling it", async () => {
    const rawSse = [
      'data: {"id":"chatcmpl-1","choices":[{"delta":{"content":"only "}}]}',
      "",
      'data: {"id":"chatcmpl-1","choices":[{"delta":{"content":"stream"},"finish_reason":"stop"}]}',
      "",
      "data: [DONE]",
      "",
    ].join("\n");
    const survivor = new Response(rawSse, {
      status: 200,
      headers: { "Content-Type": "text/event-stream", "X-Upstream-Request": "req-stream-1" },
    });
    const failed = new Response(JSON.stringify({ error: { message: "boom" } }), {
      status: 503,
      headers: { "Content-Type": "application/json" },
    });
    const handleSingleModel = vi.fn(async (panelBody, model) => {
      expect(panelBody.stream).toBe(true);
      return model === "p/ok" ? survivor : failed;
    });
    ...
    expect(res.headers.get("x-upstream-request")).toBe("req-stream-1");
    await expect(res.text()).resolves.toBe(rawSse);
  });

  it("extracts cloned SSE panel text for the judge without consuming panel streams", async () => {
    const sseResponse = (content) => new Response([
      `data: ${JSON.stringify({ choices: [{ delta: { content } }] })}`,
      "",
      "data: [DONE]",
      "",
    ].join("\n"), { headers: { "Content-Type": "text/event-stream" } });
    const handleSingleModel = vi.fn(async (_panelBody, model) => {
      if (model === "p/judge") return okResponse("FINAL");
    ...
    expect(judgeCall[0].messages.at(-1).content).toContain("stream-p/a");
    expect(judgeCall[0].messages.at(-1).content).toContain("stream-p/b");
  });
