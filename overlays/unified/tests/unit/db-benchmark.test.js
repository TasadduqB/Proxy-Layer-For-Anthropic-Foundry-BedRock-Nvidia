...
const N_ITEMS = 500;
const N_QUERIES = 200;
const RUN_BENCHMARKS = process.env.RUN_BENCHMARKS === "1";
...
beforeAll(async () => {
  ...
  // SQLite setup
  tempSqlite = fs.mkdtempSync(path.join(os.tmpdir(), "proxy-max-bench-sqlite-"));
  ...
  tempLowdb = fs.mkdtempSync(path.join(os.tmpdir(), "proxy-max-bench-lowdb-"));
...
describe.skipIf(!RUN_BENCHMARKS)("DB Benchmark — SQLite vs Lowdb", () => {
