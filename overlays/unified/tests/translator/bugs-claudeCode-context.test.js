...
import { TOOL_CONTRACT_MARKER } from "../../open-sse/guardrails/claudeToolContract.js";
...
describe("Claude Code CLI context → OpenAI", () => {
  ...
  it("tool_result image block is preserved", () => {
    ...
    expect(Array.isArray(tool?.content), "tool image should remain structured").toBe(true);
    expect(JSON.stringify(tool?.content), "image lost").toContain("IMG");
  });

  it("reinforces collaboration tool contracts for non-Anthropic models", () => {
    const out = T(FORMATS.CLAUDE, FORMATS.OPENAI, {
    ...
    const system = out.messages.find((message) => message.role === "system")?.content;
    expect(system).toContain(TOOL_CONTRACT_MARKER);
    expect(system).toContain("SendMessage requires: to, summary, message");
    expect(system).toContain("Agent requires: description, prompt, subagent_type");
    expect(system).toContain("answer the user directly in normal text");
    expect(out.tools[0].function.parameters.required).toEqual(["to", "summary", "message"]);
    expect(out.tools[1].function.parameters.required).toEqual(["description", "prompt", "subagent_type"]);
  });

  it("does not add the collaboration contract for ordinary tools", () => {
    const out = T(FORMATS.CLAUDE, FORMATS.OPENAI, {
    ...
    expect(JSON.stringify(out)).not.toContain(TOOL_CONTRACT_MARKER);
  });
});
