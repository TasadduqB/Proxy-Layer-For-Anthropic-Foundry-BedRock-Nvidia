...
describe("bug: Claude → OpenAI bridge data loss", () => {
  ...
  it("tool_result with image block is not turned into raw JSON / dropped", () => {
    ...
    expect(Array.isArray(toolMsg?.content), "tool image should remain structured").toBe(true);
    expect(JSON.stringify(toolMsg?.content), "image in tool_result lost").toContain("ZZZ");
  });

  it("tool_result error semantics survive without a nonstandard OpenAI field", () => {
    const out = T(FORMATS.CLAUDE, FORMATS.OPENAI, {
      messages: [
    ...
    expect(tool).not.toHaveProperty("is_error");
    expect(tool?.content, "model must still see that the tool failed").toContain("[Tool execution failed]");
    expect(tool?.content).toContain("boom");
  });

  it("drops is_error=false from OpenAI provider tool messages", () => {
    const out = T(FORMATS.CLAUDE, FORMATS.OPENAI, {
      messages: [
        { role: "assistant", content: [{ type: "tool_use", id: "call_1", name: "f", input: {} }] },
        { role: "user", content: [
