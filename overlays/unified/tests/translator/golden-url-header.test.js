...
const SPECIALIZED = new Set([
  "antigravity", "azure", "bedrock", "gemini-cli", "github", "iflow", "qoder", "kiro",
...
function sanitize(headers) {
  ...
  for (const [k, v] of Object.entries(headers)) {
    ...
    let value = v
      ...
      .replace(/kimi-[a-z0-9-]+/gi, "kimi-<DEVICE-ID>")
      .replace(/Proxy Max\/\d+\.\d+\.\d+/g, "Proxy Max/<VERSION>");
    if (k === "X-CLIENT-VERSION" || k === "X-CORE-VERSION" || k === "X-Msh-Version") value = "<VERSION>";
    if (k === "X-PLATFORM") value = "<PLATFORM>";
    if (k === "X-PLATFORM-VERSION") value = "<PLATFORM-VERSION>";
    if (k === "X-Msh-Device-Name") value = "<DEVICE-NAME>";
    if (k === "X-Msh-Device-Model") value = "<DEVICE-MODEL>";
    out[k] = value;
  }
  return out;
}
